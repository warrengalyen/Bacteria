...
void MeshUtilities::loadObj(const std::string & filename, Mesh & mesh, MeshUtilities::LoadMode mode)
{
	...
	while (!in.eof())
	{
		...
		// Check what kind of element the line resprents.
		...
		{ 
			...
			glm::vec3 pos = glm::vec3(std::stof(tokens[1], NULL), std::stof(tokens[2], NULL), std::stof(tokens[3], NULL));
		...
		{ 
		 // We need 3 coordinates.
			...
			glm::vec3 nor = glm::vec3(std::stof(tokens[1], NULL), std::stof(tokens[2], NULL), std::stof(tokens[3], NULL));
		...
		{ 
		 // We need 2 coordinates.
			...
			glm::vec2 uv = glm::vec2(std::stof(tokens[1], NULL), std::stof(tokens[2], NULL));
	...
	// Does the mesh have UV or normal coodinates?
	...
	else if (mode == MeshUtilities::Expanded)
	{
		...
		for (std::size_t i = 0; i < faces_temp.size(); i++)
		{
			...
			std::size_t foundF = str.find_first_of("/");
			std::size_t foundL = str.find_last_of("/");
...
void MeshUtilities::computeTangentsAndBinormals(Mesh & mesh) 
{
	...
	for (std::size_t pid = 0; pid < mesh.positions.size(); ++pid) 
	{
	...
	for (std::size_t fid = 0; fid < mesh.indices.size(); fid += 3) 
	{
	...
	for (std::size_t tid = 0; tid < mesh.tangents.size(); ++tid) 
	{
