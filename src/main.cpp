...
#define INITIAL_SIZE_WIDTH 400
#define INITIAL_SIZE_HEIGHT 400

// Used to avoid a too clean image on HiDPI screens.
...
void resize_callback(GLFWwindow* window, int width, int height) {
	...
	rendererPtr->resize(width, height, scaleFactor);
}
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(INITIAL_SIZE_WIDTH, INITIAL_SIZE_HEIGHT, "Bacteria", NULL, NULL);
	if (!window) {
	...
	// Compute scale factor based on ratio between set width (logical pixels) and obtained width (physical pixels).
	scaleFactor = width / (float)INITIAL_SIZE_WIDTH;
